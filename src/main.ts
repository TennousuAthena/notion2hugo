...
(async () => {
  ...
  myPage.results.forEach(async (page: any) => {
    ...
    await notion.pages.retrieve({ page_id: page.id }).then((pageInfo: any) => {
      console.log(
        "Processing page: " + pageInfo.properties.Name.title[0].plain_text
      );
      let description =
        typeof pageInfo.properties.Description.rich_text[0] != "undefined"
          ? pageInfo.properties.Description.rich_text[0].plain_text
          : "";
      let date = pageInfo.properties.CreatedAt.date
        ? pageInfo.properties.CreatedAt.date.start
        : pageInfo.created_time;
      let image = "";
      if (pageInfo.cover) {
      ...
      let tags = pageInfo.properties.Tags.multi_select.map(
        (tag: any) => tag.name
      );
      let tagString = "";
      ...
      let categories = "";
      let categories_array = pageInfo.properties.Categories.multi_select.map(
        (c: any) => c.name
      );
      ...
      try {
        ...
        if (count || image) {
          ...
          writeFileSync(
            "hugo/content/post/" + fileName + "/index.md",
            fileContent,
            { flag: "w" }
          );
        } else {
          writeFileSync("hugo/content/post/" + fileName + ".md", fileContent, {
            flag: "w",
          });
        }
      } catch (err) {
